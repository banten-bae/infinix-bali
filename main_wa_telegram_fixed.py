...
# Isi dengan ID chat Telegram penerima. Bisa satu ID atau beberapa ID dipisahkan koma.
...
async def evolution_webhook_handler(request):
    ...
    if event in {"MESSAGES_UPSERT", "MESSAGES_SET"}:
        ...
        if saved and telegram_application and result.get("text"):
            sent = await send_whatsapp_to_telegram(
