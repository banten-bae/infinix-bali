...
async def forward_wa_to_telegram(bot, text, wa_group="", wa_sender="", wa_number="", kab="", kec=""):
    """Kirim pesan WA ke user Telegram yang berlangganan KAB+KEC yang cocok."""
    try:
        text = str(text or "").strip()
        if not text:
            return 0

        # Kalau KAB/KEC dari webhook kosong, cari kombinasi yang memang dipasang user
        # dengan mencocokkan nama wilayah terhadap isi pesan WA.
        candidates = set()
        if kab and kec:
            candidates.update(find_users_by_kota_kec_FAST(kab, kec))
        else:
            upper_text = _norm_region_name(text)
            for key, ids in USER_CACHE.items():
                try:
                    key_kab, key_kec = key.split("|", 1)
                except ValueError:
                    continue
                if key_kab and key_kec and key_kab in upper_text and key_kec in upper_text:
                    candidates.update(ids or [])

        if not candidates:
            print(f"⚠️ Forward WA: tidak ada user cocok | kab={kab} kec={kec}")
            return 0

        safe_group = html.escape(str(wa_group or "GRUP WA"))
        safe_sender = html.escape(str(wa_sender or "-"))
        safe_number = html.escape(str(wa_number or "-"))
        safe_text = html.escape(text[:3800])
        msg = (
            "📩 <b>PESAN WHATSAPP MASUK</b>\n\n"
            f"🛡️ <b>GRUP:</b> {safe_group}\n"
            f"🧑‍💻 <b>PENGIRIM:</b> {safe_sender}\n"
            f"📱 <b>NO HP:</b> {safe_number}\n"
        )
        if kab or kec:
            msg += f"📍 <b>WILAYAH:</b> {html.escape(str(kab or '-'))} / {html.escape(str(kec or '-'))}\n"
        msg += f"\n{safe_text}"

        sent = 0
        for tid in candidates:
            try:
                await bot.send_message(int(tid), msg)
                sent += 1
                await asyncio.sleep(0.03)
            except Exception as e:
                print(f"❌ Telegram send gagal ke {tid}: {e}")
        print(f"✅ Forward WA selesai: {sent}/{len(candidates)} user")
        return sent
    except Exception as e:
        print(f"❌ forward_wa_to_telegram error: {e}")
...
async def save_whatsapp_message_evo(payload: dict):
    ...
    if not remote_jid or not text_value:
        return {"saved": False}
    push_name = data.get("pushName") or ""
    # Parser lama tidak ada di file ini. Wilayah dicocokkan langsung
    # oleh forward_wa_to_telegram dari daftar KAB+KEC user di USER_CACHE.
    kab, kec = "", ""
    try:
        conn = db()
        conn.execute("INSERT INTO wa_inbox_log (wa_group, wa_sender, message, parsed_kab, parsed_kec) VALUES (?,?,?,?,?)",
...
async def evolution_webhook_handler(request):
    ...
    if event in {"MESSAGES_UPSERT", "MESSAGES_SET"}:
        ...
        if saved and result.get("text"):
            ...
            try:
                bot = request.app['bot']
                telegram_sent = await forward_wa_to_telegram(
                    bot, result["text"], result["remote_jid"], result["push_name"], "",
                    result.get("kab", ""), result.get("kec", "")
                )
            except Exception as e:
