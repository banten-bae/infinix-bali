...
RAILWAY_URL = os.getenv("RAILWAY_URL", "https://bot-baru.up.railway.app")
...
async def callback_handler(update, context):
    ...
    if data.startswith("prov_"):
        ...
        try:
            ...
            kota_list = []
        # 1 BARIS: Kota ditampilkan horizontal
        buttons = []
        row = []
        for k in kota_list:
        ...
        if row:
            buttons.append(row)
        buttons.append([InlineKeyboardButton("⬅️ KEMBALI KE PROVINSI", callback_data="back_to_provinsi")])
        buttons.append([InlineKeyboardButton("🏠 KEMBALI KE MENU UTAMA", callback_data="back_main")])
        ...
        try:
            ...
            kec_list = []
        # 1 BARIS: Kecamatan ditampilkan horizontal
        buttons = []
        row = []
        for k in kec_list:
    ...
    elif data.startswith("back_to_kota_"):
        _, _, kota_id, kota_nama = data.split("_", 3)
        try:
            r = requests.get(f"https://www.emsifa.com/api-wilayah-indonesia/api/regencies/{kota_id}.json", timeout=10)
            kota_list = r.json()
        except:
            kota_list = []
        buttons = []
        ...
        await q.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➕ TAMBAH LAGI", callback_data="back_to_kota")],
                [InlineKeyboardButton("🏠 KEMBALI KE MENU UTAMA", callback_data="back_main")]
            ])
        )
    
    elif data == "back_to_kota":
        # Kembali ke pilihan kota
        await q.message.edit_text("📍 PILIH KOTA:", reply_markup=kb_main(uid))
